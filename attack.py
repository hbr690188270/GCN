...
# model = torch.load("./models/cora/rob_model.pt").to(device)
...
test_acc = label[test_mask].eq(y_test).sum().item()/y_test.size(0)
print("test loss: {}, test acc {}".format(test_loss, test_acc))

# pred_test_label = label[test_mask]
# pred_correct_mask = pred_test_label == y_test

# only_attack_correct = False

# if only_attack_correct:
#     attack_mask = test_mask.detach()
#     count = 0
#     for i in range(len(attack_mask)):
#         if attack_mask[i]:
#             attack_mask[i] = pred_correct_mask[count]
#             count += 1
#     attack_label = y_test[pred_correct_mask]
# else:
#     attack_mask = test_mask
#     attack_label = y_test
...
attacker.perturb(y_test = attack_label, test_mask = attack_mask, k = 200)
# attacker.perturb(y_test = y_test, test_mask = test_mask, k = 200)
# attacker.perturb(y_test = pred_test_label, test_mask = test_mask, k = 200)
